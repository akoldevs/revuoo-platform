...
type Business = {
  ...
  category: string | null; services: string[] | null; owner_id: string | null; operating_hours: any;
  reviews: Review[];
...
async function getBusinessData(slug: string): Promise<Business | null> {
    ...
    .select(`
      *, 
      reviews (*, profiles (*), business_responses (*)),
                ...
                {business.business_photos.map(photo => (
                  <div key={photo.id} className="aspect-w-1 aspect-h-1">
                     <Image 
                        ...
                        alt={photo.caption || `Photo for ${business.name}`}
                        width={400}
                        height={400}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <h2 className="text-2xl font-bold text-gray-900">
              Individual Reviews ({approvedReviews.length})
            </h2>
            ...
            {approvedReviews.length > 0 ? (
              approvedReviews.map((review) => { 
                ...
                return (
                  <Card key={review.id} className="w-full">
                    <CardHeader>
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-gray-500 mt-1">By {review.profiles?.full_name || 'Anonymous'} on {format(new Date(review.created_at), 'MMM d, yyyy')}</p>
                        </div>
                    ...
                    {approvedResponse && (
                      <CardContent>
                        <div className="bg-indigo-50 border-l-4 border-indigo-500 p-4 mt-4 rounded-r-lg">
                          <div className="flex items-start gap-3">
                            <CornerDownRight className="h-5 w-5 text-indigo-600 flex-shrink-0" />
                            <div>
                              <h4 className="font-bold text-indigo-800">Response from the business</h4>
                              <p className="text-gray-700 mt-1 italic">"{approvedResponse.response_text}"</p>
                            </div>
                          </div>
                        </div>
                      </CardContent>
              ...
              <div className="text-center py-12 bg-white rounded-lg shadow-sm">
                <p className="text-gray-600">No approved reviews yet for {business.name}.</p>
              </div>
            )}
          </div>
              ...
              <CardContent className="space-y-4 text-sm">
                  ...
                  {business.operating_hours && Object.keys(business.operating_hours).length > 0 && (
                    <div className="pt-4 border-t">
                       <h4 className="font-semibold text-gray-800 mb-2 flex items-center gap-2"><Clock className="h-4 w-4"/> Operating Hours</h4>
                            ...
                            return (
                              <div key={day} className="flex justify-between">
                                ...
                                <span className="font-medium">
                                  {hours.is_closed ? 'Closed' : `${formatTime(hours.open)} - ${formatTime(hours.close)}`}
                                </span>
                              </div>
                            )
                          })}
                  ...
                  {business.services && business.services.length > 0 && (
                    <div className="pt-4 border-t">
                       ...
                       <div className="flex flex-wrap gap-2">{business.services.map(service => (<Badge key={service} variant="secondary">{service}</Badge>))}</div></div>
                  )}
              </CardContent>
            </Card>
          </div>
