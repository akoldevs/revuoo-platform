...
import {
    Home, Wrench, CloudCog, Truck, HeartPulse, Car, Sparkles, Users, Trees, Nut, ClipboardCheck, Megaphone, Calculator, HelpCircle, Icon as LucideIcon
} from 'lucide-react';
...
const iconMap: { [key: string]: ComponentType<{ className: string }> } = {
    Sparkles, Truck, Wrench, Trees, Nut, Users, ClipboardCheck, Megaphone, Calculator, Car, HeartPulse
};

// --- Sub-Components ---
const CategoryCard = ({ category }: { category: ChildCategory }) => {
    const IconComponent = category.icon_svg ? iconMap[category.icon_svg] || HelpCircle : HelpCircle;
...
export default async function AllCategoriesPage() {
    ...
    const { data: categories, error } = await supabase.rpc('get_all_categories_hierarchical');
            ...
            <div className="bg-white border-b">
                <div className="mx-auto max-w-7xl px-6 py-24 sm:py-32 lg:px-8 text-center">
                    ...
                    <p className="mt-6 text-lg leading-8 text-gray-600">
                        Find trusted businesses and services across every category on Revuoo.
                    </p>
                </div>
            </div>
