...
// ✅ ENHANCEMENT: Centralized, robust staff verification function.
...
export async function upsertCategory(formData: FormData) {
  try {
    ...
    const categoryData = { name, slug, description, icon_svg };
...
export async function convertLeadToOpportunity(formData: FormData) {
  ...
  try {
    ...
    const { data: newOpportunity, error: opportunityError } = await supabase
      ...
      .insert({
        lead_id: lead_id,
        assigned_to_profile_id: user.id,
    ...
    const { error: logError } = await supabase.from("lead_activities").insert({
      ...
      profile_id: user.id,
      activity_type: "status_change",
      details: { old: "qualified", new: "converted" },
    });
    if (logError) throw logError;
...
export async function updateOpportunityDetails(formData: FormData) {
  ...
  try {
    ...
    // ✅ FIX: Changed 'let' to 'const' as the object is not reassigned.
    // ✅ FIX: Replaced 'any' with a specific, safe type.
    const updateData: { updated_at: string; stage?: string; notes?: string } = {
      updated_at: new Date().toISOString(),
    };
...
export async function getMessagesForChannel(channelId: number) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("messages")
      .select(`id, content, created_at, profiles ( id, full_name )`)
...
export async function getChannelMemberCount(channelId: number) {
  ...
  try {
    ...
    if (error) throw error;
    return { count };
    // ✅ FIX: Removed unused 'e' variable from the catch block
  } catch {
    return { count: 0 };
  }
}
