...
// Helper for AI safety rating badge colors
...
export default async function AdminPage() {
  ...
  // Query now also fetches the related AI analysis for each review
  ...
  const pendingResponsesPromise = supabase
    .from('business_responses')
    .select(`*, businesses (name), reviews (title)`)
    .eq('status', 'pending')
    .order('created_at', { ascending: true });

  const [{ data: pendingReviews }, { data: pendingResponses }] = await Promise.all([
    pendingReviewsPromise,
    pendingResponsesPromise
  ]);
      ...
      <div className="mt-8 space-y-6">
        ...
        {pendingReviews && pendingReviews.length > 0 ? (
          pendingReviews.map((review) => {
            const aiAnalysis = Array.isArray(review.review_ai_analysis) 
              ? review.review_ai_analysis[0] 
            ...
            return (
              <Card key={`review-${review.id}`} className="bg-white border-yellow-400">
                <CardHeader>
                    ...
                    <CardDescription>
                      For Business: <span className="font-bold text-indigo-600">{review.businesses?.name || 'N/A'}</span>
                      <br/>
                      By: <span className="font-semibold">{review.profiles?.full_name || 'Anonymous'}</span> on {format(new Date(review.created_at), 'MMM d, yyyy')}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      <div className="border-t pt-4">
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3 text-sm">
                           <div>
                              ...
                              <Badge variant={getSafetyBadgeVariant(aiAnalysis.safety_rating)}>
                                {aiAnalysis.safety_rating || 'N/A'}
                              </Badge>
                           </div>
          ...
          pendingResponses.map((response) => (
            <Card key={`response-${response.id}`} className="bg-white border-blue-400">
              <CardHeader>
                ...
                <CardDescription>
                  For Business: <span className="font-bold text-indigo-600">{response.businesses?.name}</span>
                </CardDescription>
              </CardHeader>
              <CardContent>
                <p className="italic bg-gray-50 p-4 rounded-md">{response.response_text}</p>
              </CardContent>
