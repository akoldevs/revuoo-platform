...
type PendingReview = {
  ...
  review_ai_analysis:
    | {
        safety_rating?: "SAFE" | "FLAGGED" | string;
        reasoning?: string;
        sentiment?: string;
        summary?: string;
      }[]
...
export default async function AdminPage() {
  ...
  const pendingReviewsPromise = supabase
    .from("reviews")
    .select(
      `*, businesses (name), profiles (full_name), review_ai_analysis (*)`
    )
    .eq("status", "pending")
  ...
  const [{ data: pendingReviews }, { data: pendingResponses }] =
    await Promise.all([pendingReviewsPromise, pendingResponsesPromise]);
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-6 py-12">
          ...
          pendingReviews.map((review: PendingReview) => {
            ...
            return (
              ...
              >
                ...
                <CardContent>
                  <p className="italic bg-gray-50 p-4 rounded-md mb-6">
                    {review.summary}
                  </p>
                  {aiAnalysis && (
      ...
      <div className="mt-12 pt-8 border-t space-y-6">
        <h2 className="text-2xl font-semibold">
          Pending Business Responses ({pendingResponses?.length || 0})
        </h2>
        {pendingResponses && pendingResponses.length > 0 ? (
          pendingResponses.map((response: PendingResponse) => (
            <Card
              key={`response-${response.id}`}
              className="bg-white border-blue-400"
            >
              <CardHeader>
                <CardTitle>
                  Response to: &quot;{response.reviews?.title}&quot;
                </CardTitle>
                <CardDescription>
                  For Business:{" "}
                  <span className="font-bold text-indigo-600">
              ...
              <CardContent>
                <p className="italic bg-gray-50 p-4 rounded-md">
